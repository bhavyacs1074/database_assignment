...
/* Extended version: allows choosing replacement policy (LRU or MRU) */
PF_OpenFileEx(fname, policy)
char *fname;
int policy;   /* PF_REPL_LRU or PF_REPL_MRU */
{
    int fd;
    ...
    /* store chosen policy in open-file table */
    if (policy != PF_REPL_LRU && policy != PF_REPL_MRU)
        policy = PF_REPL_LRU;  /* default */

    PFftab[fd].policy = policy;
...
/* Return the replacement policy for a file descriptor */
int PF_GetPolicy(fd)
int fd;
{
    if (PFinvalidFd(fd))
        return PF_REPL_LRU;  /* default safe fallback */
    return PFftab[fd].policy;
}
