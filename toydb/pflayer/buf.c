...
static PFbufInternalAlloc(fd,bpage,writefcn)
int fd;
...
int error;		/* error value returned*/
    int policy = PF_REPL_LRU;  /* default */
    int fd_for_policy = -1;
    fd_for_policy = fd;
    policy = PF_GetPolicy(fd_for_policy);
...
/* choose victim based on the file’s replacement policy */
if (policy == PF_REPL_LRU) {
    /* LRU: search from the tail backward */
    for (tbpage = PFlastbpage; tbpage != NULL; tbpage = tbpage->prevpage) {
        if (!tbpage->fixed) { victim = tbpage; break; }
    }
} else { /* MRU */
    /* MRU: search from the head forward */
...
tbpage = victim;   /* continue with the existing logic below */
...
int error;
	...
	if ((bpage=PFhashFind(fd,pagenum)) == NULL){
		...
		if ((error=PFbufInternalAlloc(fd,&bpage,writefcn))!= PFE_OK){
			/* error */
			*fpage = NULL;
			return(error);
		}
